...
export class AssignedRoleActions {
    ...
    static async fetchUserRoles() {
        let url = Endpoints.AssignedRoleManager.GET.getAssignedRoles();
        try {
    ...
    static async updateUserRoles(assignedRolesModel: AssignedRole){
        let url = Endpoints.AssignedRoleManager.POST.updateAssignedRole();
        try {
            let res: any = await Http.post(url, assignedRolesModel);
            return res;
        } catch (error) {
         console.log(error);
        }
