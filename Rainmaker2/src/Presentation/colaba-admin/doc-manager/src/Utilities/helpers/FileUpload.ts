...
export class FileUpload {
  ...
  static getMimetype(signature: any) {
    switch (signature) {
      case this.PNG.hex:
  ...
  static getActualMimeType(file:any): Promise<string> {
  ...
  static async isTypeAllowed(file: File) {
    const result = this.allowedFileTypes.includes(await this.getActualMimeType(file));
    return result;
  }

  static isSizeAllowed(file: any) {
    return IsSizeAllowed(file, this.allowedSize);
  }

  static getExtension(file: { clientName: string; type: string; }, splitBy: string) {
    if (splitBy === "dot") {
  ...
  static getDocLogo(file: any, splitBy: any) {
  ...
  static getFileNameByDate(){
    let d = new Date();
    let test =  d.getDate().toString() + d.getMonth().toString() + d.getFullYear().toString() + d.getHours().toString() + d.getMinutes().toString() + d.getSeconds().toString()+ '.pdf'
