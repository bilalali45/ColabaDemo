...
} from "../TemplateListContainer/TemplateListContainer";
...
export const SelectedTemplate = ({
  ...
  useEffect(() => {
    let nameUsed = templates?.filter(
      (t: Template) =>
        t.name.toLowerCase().includes("new template") &&
        !isNaN(parseInt(t.name.split(" ")[2]))
    )?.length;
  ...
  const addNewTemplate = async (name: string) => {
    ...
    if (insertedTemplate) {
      ...
      let currentTemplate = updatedTemplates.find(
        (t: Template) => t.name === name
      );
      dispatch({
      ...
      if (listContainerElRef?.current) {
        listContainerElRef.current.scrollTo(
          0,
          listContainerElRef.current?.children[0]?.clientHeight + 40
        );
      }
    }
  };
  ...
  const renameTemplate = async (value: string) => {
    ...
    if (
      templates.find(
        (t: Template) =>
          t.name.trim() === value.trim() && t.id !== currentTemplate?.id
      )
    ) {
              ...
              <li key={td.docId}>
                <p title={td.docName}>
                  ...
                  td.docId === removeDocName ? (
                    ...
                    currentTemplate?.type === MyTemplate && (
                      <span title="Remove" className="BTNclose">
                        <i
                          className="zmdi zmdi-close"
                          onClick={() =>
                            removeDoc(currentTemplate?.id, td?.docId)
                          }
                        ></i>
                      </span>
                    )
  ...
  return (
    <section className="veiw-SelectedTemplate">
      ...
      {(templates && !currentTemplate) || templateDocuments?.length === 0 ? (
        <NewTemplate setLoaderVisible={setLoaderVisible} />
      ) : currentTemplate && templateDocuments?.length ? (
        renderDocumentList()
      ) : (
        <Loader containerHeight={"100%"} />
      )}
