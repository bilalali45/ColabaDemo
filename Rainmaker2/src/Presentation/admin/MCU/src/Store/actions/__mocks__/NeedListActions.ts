...
const autoSyncData = {
  autoSyncToBytePro: 0,
  emailTemplate: "Hi {user},↵↵We're busy reviewing your file, but to continue the process we need the following documents.↵↵{documents}↵↵Please upload the above documents in two days to avoid any processing delays.↵↵Thank you,",
...
export class NeedListActions {
  ...
  static async checkIsByteProAuto() {
    try {
      return Promise.resolve(autoSyncData);
    } catch (error) {
      console.log(error);
    }
  }
