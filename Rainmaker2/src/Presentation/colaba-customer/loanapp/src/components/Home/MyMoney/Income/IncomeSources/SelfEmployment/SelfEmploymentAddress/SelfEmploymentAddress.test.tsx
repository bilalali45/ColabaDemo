...
import { SelfEmploymentAddress } from "./SelfEmploymentAddress";
...
describe("Personal information Subject Home Address ", () => {
  test("Should change street Address", async () => {
    const { getAllByTestId } = render(
      <Store.Provider value={{ state, dispatch }}>
        <SelfEmploymentAddress selfIncome={{jobTitle:''}} setSelfIncomeFormData={{}} updateFormValuesOnChange={()=>{}} />
      </Store.Provider>
    );
  ...
  test("Should submit form if all fields are filled", async () => {
    ...
    let SubmitBtn;
    await waitFor(() => {
      SubmitBtn = getByTestId("self-employment-address-next");
      //
