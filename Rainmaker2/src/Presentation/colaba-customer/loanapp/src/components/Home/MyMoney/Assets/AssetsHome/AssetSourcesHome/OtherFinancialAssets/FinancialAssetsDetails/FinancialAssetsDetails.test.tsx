...
import {FinancialAssetsDetails} from './FinancialAssetsDetails';
...
beforeEach(() => {
  ...
  describe("Financial Asset Detail", () => {
    test("Should render Financial Asset Detail", async () => {
      const { getByTestId , getAllByTestId} = render(
        <Store.Provider value={{ state, dispatch }}>
          <FinancialAssetsDetails />
          </Store.Provider>
      );

     
      await waitFor(() => {      
        expect(getByTestId('financial-form')).toBeInTheDocument();
        expect(getByTestId('financialInstitution-input')).toBeInTheDocument();
        expect(getByTestId('accountNumber-input')).toBeInTheDocument();
        expect(getByTestId('currentBalance-input')).toBeInTheDocument();
        expect(screen.queryByTestId('save-msg')).toBeNull();
        expect(getByTestId('saveBtn')).toBeInTheDocument();
        expect(screen.queryByTestId('saved-actions')).toBeNull();

        });
        let instituteInput, accountInput, balanceInput ;

        await waitFor(() => { 
          instituteInput = getByTestId('financialInstitution-input');
          accountInput = getByTestId('accountNumber-input');
          balanceInput = getByTestId('currentBalance-input');

          fireEvent.change(instituteInput, { target: { value: "Stock Investment" } });
          fireEvent.change(accountInput, { target: { value: "4855566666" } });
          fireEvent.change(balanceInput, { target: { value: "500000" } });
        });

        await waitFor(() => {
          expect(instituteInput).toHaveDisplayValue("Stock Investment");
          expect(accountInput).toHaveDisplayValue("4855566666");
          expect(balanceInput).toHaveDisplayValue("500,000");
        });
        ...
        await waitFor(() => {
        expect(screen.queryByTestId('save-msg')).not.toBeNull();
        expect(screen.queryByTestId('saveBtn')).toBeNull();
        expect(screen.queryByTestId('saved-actions')).not.toBeNull();
        });
       
        let addAnother = getByTestId('AddAnotherFinancialAccountBtn');
        fireEvent.click(addAnother);

      });

      test("Should click AddAnotherAssetBtn", async () => {
        const { getByTestId , getAllByTestId} = render(
          <Store.Provider value={{ state, dispatch }}>
            <FinancialAssetsDetails />
            </Store.Provider>
        ...
        await waitFor(() => {      
          ...
          fireEvent.change(instituteInput, { target: { value: "Stock Investment" } });
          fireEvent.change(accountInput, { target: { value: "4855566666" } });
          fireEvent.change(balanceInput, { target: { value: "870000" } });
          });
  
          await waitFor(() => {
            expect(instituteInput).toHaveDisplayValue("Stock Investment");
            expect(accountInput).toHaveDisplayValue("4855566666");
            expect(balanceInput).toHaveDisplayValue("870,000");
          });
          ...
          let addAnother = getByTestId('AddAnotherAssetBtn');
          fireEvent.click(addAnother);
