...
const state = {
  ...
  describe("Consent Screen", () => {
    test("Should render Consent List", async () => {
        ...
        await waitFor(() => {                       
            ...
            expect(getByTestId('consent-tab-screen')).toBeInTheDocument();
            expect(getAllByTestId('agreement-screen')).toHaveLength(2);

            acceptBtns = getAllByTestId('acceptBtn');
            expect(acceptBtns[0]).toBeDisabled();

            checkBoxes = getAllByTestId('InputCheckedBox');
            fireEvent.click(checkBoxes[0]);
        });

        await waitFor(() => {
        expect(acceptBtns[0]).not.toBeDisabled();
