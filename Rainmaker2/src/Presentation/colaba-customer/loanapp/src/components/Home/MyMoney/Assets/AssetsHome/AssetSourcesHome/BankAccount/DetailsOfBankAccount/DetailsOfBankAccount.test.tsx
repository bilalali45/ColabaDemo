...
import {DetailsOfBankAccount} from './DetailsOfBankAccount';
...
beforeEach(() => {
  ...
  describe("Bank Account Detail", () => {
    test("Should render Bank Account Detail", async () => {
      ...
      await waitFor(() => {      
        expect(getByTestId('bank-form')).toBeInTheDocument();
        expect(getByTestId('sub-title')).toHaveTextContent('What type of account is it?');
        ...
        await waitFor(() => { 
          ...
          fireEvent.change(instituteInput, { target: { value: "United State Bank Ltd" } });
          fireEvent.change(accountInput, { target: { value: "85789955555" } });
          fireEvent.change(balanceInput, { target: { value: "50000" } });
        });

        await waitFor(() => {
          expect(instituteInput).toHaveDisplayValue("United State Bank Ltd");
          expect(accountInput).toHaveDisplayValue("85789955555");
          expect(balanceInput).toHaveDisplayValue("50,000");
        });
        ...
        let addAnother = getByTestId('AddAnotherBankAccountBtn');
