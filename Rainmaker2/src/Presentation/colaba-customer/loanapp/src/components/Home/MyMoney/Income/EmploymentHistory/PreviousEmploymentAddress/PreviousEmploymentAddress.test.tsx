...
import { PreviousEmploymentAddress } from "./PreviousEmploymentAddress";
...
describe("Personal information Subject Home Address ", () => {
  ...
  test("Should show errors on empty form", async () => {
    const { getByTestId } = render(<PreviousEmploymentAddress />);

    let SubmitBtn:HTMLElement;
    await waitFor(() => {
      SubmitBtn = getByTestId("previous-employment-address-next");
      //
  ...
  test("Should submit form if all fields are filled", async () => {
    const { getByTestId, getAllByTestId } = render(<PreviousEmploymentAddress />);
    await waitFor(() => {
      ...
      let stateName:HTMLElement = getByTestId("state");
      let stateTxt = stateName.children[0].children[0];
      fireEvent.change(stateTxt, { target: { value: "Texas" } });

      fireEvent.change(getByTestId("Zip-Code"), { target: { value: "78717" } });

      let country:HTMLElement = getByTestId("country");
      // expect(resetPassHeader).toBeInTheDocument();
      let countryTxt = country.children[0].children[0];
      fireEvent.change(countryTxt, { target: { value: "United States" } });
    });
    let SubmitBtn:HTMLElement;
    await waitFor(() => {
