...
namespace Rainmaker.Service
{
    public class ActivityService : ServiceBase<RainMakerContext, Activity>, IActivityService
    {
        ...
        {
            var activity = Uow.Repository<Activity>().Query(item => (item.BusinessUnitId == null || item.BusinessUnitId == businessUnitId) && item.ActivityForId == (int)activityFor && (item.IsCustomerDefault != null && item.IsCustomerDefault.Value) && item.IsActive && !item.IsDeleted)
