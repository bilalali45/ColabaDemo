...
namespace RainMaker.Data.Mapping
{
    ...
    public partial class CurrentRateMap : Microsoft.EntityFrameworkCore.IEntityTypeConfiguration<CurrentRate>
    {
        public void Configure(EntityTypeBuilder<CurrentRate> builder)
        {
            builder.ToTable("CurrentRate", "dbo");
            ...
            builder.Property(x => x.RateArchiveId).HasColumnName(@"RateArchiveId").HasColumnType("int").IsRequired(false);
            ...
            builder.Property(x => x.RateParameterId).HasColumnName(@"RateParameterId").HasColumnType("int").IsRequired();
            ...
            // Foreign keys
            builder.HasOne(a => a.BusinessUnit).WithMany(b => b.CurrentRates).HasForeignKey(c => c.BusinessUnitId).OnDelete(DeleteBehavior.SetNull); // FK_CurrentRate_BusinessUnit
            builder.HasOne(a => a.Investor).WithMany(b => b.CurrentRates).HasForeignKey(c => c.InvestorId).OnDelete(DeleteBehavior.SetNull); // FK_CurrentRate_Investor
            builder.HasOne(a => a.Product).WithMany(b => b.CurrentRates).HasForeignKey(c => c.ProductId).OnDelete(DeleteBehavior.SetNull); // FK_CurrentRate_Product
            builder.HasOne(a => a.RateServiceParameter).WithMany(b => b.CurrentRates).HasForeignKey(c => c.RateParameterId).OnDelete(DeleteBehavior.SetNull); // FK_CurrentRate_RateServiceParameter
            InitializePartial();
        }
