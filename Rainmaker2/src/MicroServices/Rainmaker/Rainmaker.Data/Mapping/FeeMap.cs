...
namespace RainMaker.Data.Mapping
{
    ...
    public partial class FeeMap : Microsoft.EntityFrameworkCore.IEntityTypeConfiguration<Fee>
    {
        public void Configure(EntityTypeBuilder<Fee> builder)
        {
            ...
            builder.Property(x => x.FeeCategoryId).HasColumnName(@"FeeCategoryId").HasColumnType("int").IsRequired(false);

            // Foreign keys
            builder.HasOne(a => a.FeeBlock).WithMany(b => b.Fees).HasForeignKey(c => c.FeeBlockId).OnDelete(DeleteBehavior.SetNull); // FK_Fee_FeeBlock
            builder.HasOne(a => a.FeeCategory).WithMany(b => b.Fees).HasForeignKey(c => c.FeeCategoryId).OnDelete(DeleteBehavior.SetNull); // FK_Fee_FeeCategory
            builder.HasOne(a => a.FeeType).WithMany(b => b.Fees).HasForeignKey(c => c.FeeTypeId).OnDelete(DeleteBehavior.SetNull); // FK_Fee_FeeType
            InitializePartial();
        }
