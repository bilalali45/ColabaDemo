...
namespace LosIntegration.API.Controllers
{
    ...
    [ApiController]
    public class SubPropertyController : ControllerBase
    {
        #region Constructors

        public SubPropertyController(IHttpClientFactory clientFactory,
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(ByteSubProperty byteSubProperty)
        {
            var rainmakerSubProperty = byteSubProperty.GetRainmakerSubProperty();

            var content = rainmakerSubProperty.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "RainMaker:Url"]}/api/rainmaker/SubProperty/AddOrUpdate",
