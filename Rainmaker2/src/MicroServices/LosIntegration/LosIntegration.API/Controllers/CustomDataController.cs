...
namespace LosIntegration.API.Controllers
{
    ...
    public class CustomDataController : ControllerBase
    {
        ...
        public CustomDataController(IHttpClientFactory clientFactory,
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(ByteCustomValue byteCustomValue)
        {
            var rainmakerCustomValue = byteCustomValue.GetRainmakerCustomValue();

            var content = rainmakerCustomValue.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "RainMaker:Url"]}/api/rainmaker/CustomData/AddOrUpdate",
