...
namespace LoanApplicationDb.Data.Mapping
{
    ...
    public partial class IncomeInfoMap : Microsoft.EntityFrameworkCore.IEntityTypeConfiguration<IncomeInfo>
    {
        public void Configure(EntityTypeBuilder<IncomeInfo> builder)
        {
            builder.ToTable("IncomeInfo", "dbo");
            ...
            builder.Property(x => x.EmployerName).HasColumnName(@"EmployerName").HasColumnType("nvarchar").IsRequired(false).HasMaxLength(150);
            ...
            builder.Property(x => x.MonthlyBaseIncome).HasColumnName(@"MonthlyBaseIncome").HasColumnType("decimal(18,2)").IsRequired();
            ...
            builder.Property(x => x.AnnualBaseIncome).HasColumnName(@"AnnualBaseIncome").HasColumnType("decimal(18,2)").IsRequired();
            builder.Property(x => x.IncomeTypeId).HasColumnName(@"IncomeTypeId").HasColumnType("int").IsRequired();
            builder.Property(x => x.HourlyRate).HasColumnName(@"HourlyRate").HasColumnType("decimal(18,2)").IsRequired(false);
            builder.Property(x => x.HoursPerWeek).HasColumnName(@"HoursPerWeek").HasColumnType("int").IsRequired(false);
            builder.Property(x => x.Description).HasColumnName(@"Description").HasColumnType("nvarchar").IsRequired(false).HasMaxLength(500);

            // Foreign keys
            builder.HasOne(a => a.AddressInfo).WithMany(b => b.IncomeInfoes).HasForeignKey(c => c.EmployerAddressId).OnDelete(DeleteBehavior.SetNull); // FK_EmploymentInfo_AddressInfo
            builder.HasOne(a => a.Borrower).WithMany(b => b.IncomeInfoes).HasForeignKey(c => c.BorrowerId).OnDelete(DeleteBehavior.SetNull); // FK_EmploymentInfo_Borrower
            builder.HasOne(a => a.IncomeType).WithMany(b => b.IncomeInfoes).HasForeignKey(c => c.IncomeTypeId).OnDelete(DeleteBehavior.SetNull); // FK_IncomeInfo_IncomeType_Id
            builder.HasOne(a => a.JobType).WithMany(b => b.IncomeInfoes).HasForeignKey(c => c.JobTypeId).OnDelete(DeleteBehavior.SetNull); // FK_EmploymentInfo_JobType
            builder.HasOne(a => a.LoanContact).WithMany(b => b.IncomeInfoes).HasForeignKey(c => c.ContactPersonId).OnDelete(DeleteBehavior.SetNull); // FK_EmploymentInfo_LoanContact
            InitializePartial();
        }
