...
namespace ByteWebConnector.API.Controllers
{
    ...
    public class BorrowerIncomeController : ControllerBase
    {
        ...
        {
            var borrowerIncome = byteIncome.GetBorrowerIncome();

            var content = borrowerIncome.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "LosIntegration:Url"]}/api/LosIntegration/BorrowerIncome/AddOrUpdate",
