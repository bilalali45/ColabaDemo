...
namespace LosIntegration.API.Controllers
{
    ...
    [ApiController]
    public class BorrowerIncomeController : ControllerBase
    {
        #region Constructors

        public BorrowerIncomeController(IHttpClientFactory clientFactory,
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(ByteIncome byteIncome)
        {
            var rainmakerBorrowerIncome = byteIncome.GetRainmakerBorrowerIncome();

            var content = rainmakerBorrowerIncome.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "RainMaker:Url"]}/api/rainmaker/BorrowerIncome/AddOrUpdate",
