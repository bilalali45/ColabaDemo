...
namespace ByteWebConnector.API.Controllers
{
    ...
    public class BorrowerLiabilitiesController : ControllerBase
    {
        ...
        {
            var borrowerLiability = byteLiability.GetBorrowerLiability();

            var content = borrowerLiability.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "LosIntegration:Url"]}/api/LosIntegration/BorrowerLiability/AddOrUpdate",
