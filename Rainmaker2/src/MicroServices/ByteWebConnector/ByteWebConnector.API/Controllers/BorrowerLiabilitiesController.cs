...
namespace ByteWebConnector.API.Controllers
{
    ...
    [ApiController]
    public class BorrowerLiabilitiesController : ControllerBase
    {
        #region Constructors

        public BorrowerLiabilitiesController(IHttpClientFactory clientFactory,
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(ByteLiability byteLiability)
        {
            var rainmakerBorrowerLiability = byteLiability.GetRainmakerBorrowerLiability();

            var content = rainmakerBorrowerLiability.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "RainMaker:Url"]}/api/rainmaker/BorrowerLiability/AddOrUpdate",
