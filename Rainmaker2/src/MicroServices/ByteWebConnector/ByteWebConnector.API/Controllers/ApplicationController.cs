...
namespace LosIntegration.API.Controllers
{
    ...
    public class ApplicationController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(ByteApplication byteApplication)
        {
            var rainmakerApplication = byteApplication.GetRainmakerApplication();

            var content = rainmakerApplication.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "RainMaker:Url"]}/api/rainmaker/Application/AddOrUpdate",
