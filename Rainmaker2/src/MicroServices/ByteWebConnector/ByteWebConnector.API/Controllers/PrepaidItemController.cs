...
namespace ByteWebConnector.API.Controllers
{
    ...
    [ApiController]
    public class PrepaidItemController : ControllerBase
    {
        #region Constructors

        public PrepaidItemController(IHttpClientFactory clientFactory,
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(BytePrepaidItem bytePrepaidItem)
        {
            var rainmakerPrepaidItem = bytePrepaidItem.GetRainmakerPrepaidItem();

            var content = rainmakerPrepaidItem.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "LosIntegration:Url"]}/api/LosIntegration/PrepaidItem/AddOrUpdate",
