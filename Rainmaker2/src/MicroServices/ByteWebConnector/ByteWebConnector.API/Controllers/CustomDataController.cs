...
namespace ByteWebConnector.API.Controllers
{
    ...
    public class CustomDataController : ControllerBase
    {
        ...
        {
            var customValue = byteCustomValue.GetCustomValue();

            var content = customValue.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "LosIntegration:Url"]}/api/LosIntegration/CustomData/AddOrUpdate",
