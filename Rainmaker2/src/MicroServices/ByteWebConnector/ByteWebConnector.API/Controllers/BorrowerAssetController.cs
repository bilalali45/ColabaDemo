...
namespace ByteWebConnector.API.Controllers
{
    ...
    [ApiController]
    public class BorrowerAssetController : ControllerBase
    {
        #region Constructors

        public BorrowerAssetController(IHttpClientFactory clientFactory,
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(ByteAsset byteAsset)
        {
            var rainmakerBorrowerAsset = byteAsset.GetRainmakerBorrowerAsset();

            var content = rainmakerBorrowerAsset.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "RainMaker:Url"]}/api/rainmaker/BorrowerAsset/AddOrUpdate",
