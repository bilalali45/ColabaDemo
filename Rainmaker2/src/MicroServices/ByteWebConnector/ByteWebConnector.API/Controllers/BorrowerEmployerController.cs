...
namespace ByteWebConnector.API.Controllers
{
    ...
    public class BorrowerEmployerController : ControllerBase
    {
        ...
        {
            var borrowerEmployer = byteEmployer.GetBorrowerEmployer();

            var content = borrowerEmployer.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "LosIntegration:Url"]}/api/LosIntegration/BorrowerEmployer/AddOrUpdate",
