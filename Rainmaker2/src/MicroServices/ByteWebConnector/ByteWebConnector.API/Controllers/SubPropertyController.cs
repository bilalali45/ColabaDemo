...
namespace ByteWebConnector.API.Controllers
{
    ...
    public class SubPropertyController : ControllerBase
    {
        ...
        {
            var subProperty = byteSubProperty.GetSubProperty();

            var content = subProperty.ToJsonString();

            var callResponse =
                await _httpClient.PostAsync(requestUri:
                                           $"{_configuration[key: "LosIntegration:Url"]}/api/LosIntegration/SubProperty/AddOrUpdate",
