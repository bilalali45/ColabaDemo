...
//        tableViewOverView.register(UINib(nibName: "BorrowerApplicationStatusButtonTableViewCell", bundle: nil), forCellReuseIdentifier: "BorrowerApplicationStatusButtonTableViewCell")
    ...
    func getBorrowerInvitationStatus(){
        ...
        let extraData = "borrowerId=\(loanInfoData.borrowers.first!.borrowerId)&loanapplicationid=\(loanApplicationId)"
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if (indexPath.row == 0){
            if (invitationStatus == "Invite Borrower" || invitationStatus == "Invitation Pending" || invitationStatus == "Invitation Resent"){
                ...
                vc.loanApplicationId = self.loanApplicationId
                vc.borrowerId = self.loanInfoData.borrowers.first!.borrowerId
